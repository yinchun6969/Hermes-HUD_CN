...
- retaining compatibility with standard Hermes data directories
...
If you use this project publicly, keep your own `HERMES_HOME`, credentials, and runtime data outside this repository.
...
13 tabs covering identity, memory, skills, sessions, cron jobs, projects, health, agents, chat, profiles, costs, corrections, and patterns.

共 13 个主要标签页，覆盖身份信息、记忆、技能、会话、定时任务、项目、健康、代理、聊天、画像、成本、纠错和模式分析。
...
This is the browser companion to [hermes-hud](https://github.com/joeynyc/hermes-hud). Both read from the same `~/.hermes/` data directory independently.

这是 [hermes-hud](https://github.com/joeynyc/hermes-hud) 的浏览器版配套界面。两者都读取同一个 `~/.hermes/` 数据目录，但彼此独立运行。

The Web UI adds dedicated Memory, Skills, Sessions, Costs, and live Chat views on top of the terminal experience.

相比终端界面，Web UI 额外提供了更清晰的 Memory、Skills、Sessions、Costs 和实时 Chat 视图。

If you also use the TUI, you can enable it with:
